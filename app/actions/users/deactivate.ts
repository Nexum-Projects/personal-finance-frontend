...
export default async function deactivateUser(userId: string): Promise<ActionResponse<User>> {
  try {
    ...
    const response = await baseAxios.patch<{ data?: User }>(`/users/${userId}/deactivate`)
